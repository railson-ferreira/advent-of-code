...
func main() {
	...
	minLocation := -1
	for _, seed := range theAlmanac.seeds {
		soil := theAlmanac.findSoil(seed)
...
func formatInput(input string) (theAlmanac almanac) {
	...
	for i, line := range lines {
		...
		if i == 0 {
			for _, seedText := range strings.Split(strings.Split(line, ":")[1], " ") {
				if len(seedText) == 0 {
					continue
				}
				seedNumber, _ := strconv.Atoi(seedText)
				theAlmanac.seeds = append(theAlmanac.seeds, seedNumber)
			}
			continue
		}
