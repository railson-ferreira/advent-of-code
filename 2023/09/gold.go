...
func main() {
	...
	for _, sequence := range sequences {
		sum += findTheExtrapolateNumberBackwards(sequence)
	}
...
func findTheExtrapolateNumberBackwards(sequence []int) int {
	diffSequence := getDiffSequence(sequence)
	if areAllZeros(diffSequence) {
		return sequence[0]
	}
	diffExtrapolateNumber := findTheExtrapolateNumberBackwards(diffSequence)

	return sequence[0] - diffExtrapolateNumber
}
