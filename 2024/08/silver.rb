  ...
  antennas_by_char.each do |key_value|
    ...
    locations.each_with_index do |location_a, index_a|
      locations[index_a + 1..].each do |location_b|
        ...
        if antinode_a.row >= 0 && antinode_a.column >= 0 && antinode_a.row < height && antinode_a.column < width
          antinodes_location_set.add(antinode_a)
        end
        if antinode_b.row >= 0 && antinode_b.column >= 0 && antinode_b.row < height && antinode_b.column < width
